/*
  40-Yard Dash Timer — Finish Gate (ESP32)
  - Detects beam break via phototransistor circuit (when hardware exists)
...
// ===================== USER CONFIG =====================
// Placeholder MAC. Replace later when you have hardware.
...
// Packet config
static const uint8_t GATE_ID_FINISH = GATE_FINISH;
...
static bool sendFinishPacket() {
  TimerPacket pkt;
  pkt.gate_id = GATE_ID_FINISH;
...
void setup() {
...
#if SIM_MODE
  Serial.println();
  Serial.println("=== Finish Gate (SIM MODE) ===");
  Serial.println("Type 'b' in Serial Monitor to simulate a beam break (FINISH).");
  ...
  Serial.println();
  Serial.println("=== Finish Gate Boot ===");
  Serial.print("My MAC: ");
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (c == 'b' || c == 'B') {
      seqNum++;
      Serial.print("SIM: Beam break -> FINISH event, seq=");
      Serial.print(seqNum);
  ...
  if (armed && beamBroken && (now - stateChangeMs >= DEBOUNCE_MS)) {
    bool ok = sendFinishPacket();
    Serial.print("FINISH trigger sent. ok=");
